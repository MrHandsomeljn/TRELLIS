	...
	bool antialiasing)
{
	...
	if (dL_dabsdepth)
	dL_dtz -= dL_dabsdepth[idx] / (t.z * t.z);
...
renderCUDA(
	...
	const float* __restrict__ dL_absdepths,
	float3* __restrict__ dL_dmean2D,
	float4* __restrict__ dL_dconic2D,
	float* __restrict__ dL_dopacity,
	float* __restrict__ dL_dcolors,
	float* __restrict__ dL_dinvdepths,
	float* __restrict__ dL_dabsdepths
)
{
	...
	__shared__ float collected_depths_abs[BLOCK_SIZE];
    ...
    float dL_dpixel_abs[C];
    float dL_absdepth;
	float accum_absdepth_rec = 0;
	if (inside)
	{
		for (int i = 0; i < C; i++)
        dL_dpixel_abs[i] = dL_dpixels[i * H * W + pix_id];
		if(dL_absdepths)
		dL_absdepth = dL_absdepths[pix_id];
	}

	float last_alpha_abs = 0;
	float last_color_abs[C] = { 0 };
	float last_absdepth = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
            ...
            if(dL_absdepths)
			collected_depths_abs[block.thread_rank()] = depths[coll_id];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
            ...
            if (dL_dabsdepths)
			{
			const float absd = collected_depths_abs[j];
			accum_absdepth_rec = last_alpha * last_absdepth + (1.f - last_alpha) * accum_absdepth_rec;
			last_absdepth = absd;
			dL_dalpha += (absd - accum_absdepth_rec) * dL_absdepth;
			atomicAdd(&(dL_dabsdepths[global_id]), dchannel_dcolor * dL_absdepth);
			}
